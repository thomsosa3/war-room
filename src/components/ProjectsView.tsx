...
export default function ProjectsView() {
  ...
  const toggleMaterial = (projectId: string, materials: Material[], m: Material) => {
    updateProject(projectId, { materials: materials.map((x) => (x.id === m.id ? { ...x, bought: !x.bought } : x)) });
  };
      ...
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        {projects.length === 0 ? (
          <div className="rounded-lg border border-dashed border-ground-line px-4 py-10 text-center text-sm text-ink-faint">
            No projects yet. Make one (e.g. <span className="text-ink">Fence</span>) and tag tasks to it.
          </div>
        ) : (
              ...
              {shopping.map(({ project, items }) => (
                <div key={project.id}>
                  ...
                  <div className="space-y-1">
                    {items.map((m) => (
                      <label key={m.id} className="flex cursor-pointer items-center gap-2 rounded-md border border-ground-line bg-ground-raised px-3 py-1.5 text-sm">
                        <input type="checkbox" checked={false} onChange={() => toggleMaterial(project.id, project.materials ?? [], m)} className="h-4 w-4 accent-pine" />
